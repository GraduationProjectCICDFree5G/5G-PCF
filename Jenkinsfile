pipeline {
    ...
    stages {
        ...
        stage('docker build') {
            steps {
                sh(script: """
                    docker images -a
                    docker build -t 5ggraduationproject/5g-pcf:latest . 
                    docker images -a
                """)
            }
        }
        ...
        stage('Scan Image for Common Vulnerabilities and Exposures') {
            steps {
                sh 'trivy image 5ggraduationproject/5g-pcf --output trivy-report.json'
            }
        }
        stage('Pushing to Dockerhub') {
            steps {
                sh 'docker push 5ggraduationproject/5g-pcf:latest'
            }
        }
        ...
        stage('Deploy Helm Chart on EKS') {
            steps {
                sh 'helm upgrade --install pcf ./helm/'
            }
        }
    }
    ...
    post {
        always {
            ...
            sh 'docker rmi 5ggraduationproject/5g-pcf'
            sh 'docker rmi 5ggraduationproject/pcf-base'
        }
    }
}